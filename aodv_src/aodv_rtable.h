...
/*
  ...
  邻居缓存项用于存储 AODV 路由表中的**下一跳节点的信息**，包括下一跳节点的 IP
  地址、有效期等信息。
*/
class AODV_Neighbor {
  // 通过将这两个类声明为 AODV_Neighbor 的友元类，可以让它们能够访问和修改
  // AODV_Neighbor 类中的私有成员
  friend class AODV; // AODV 是 AODV 协议的主类，用于实现 AODV 协议的主要功能
  friend class aodv_rt_entry; // aodv_rt_entry 路由表类，用于存储和管理 AODV
                              // 路由表中的路由信息
...
protected:
  // 该宏将 AODV_Neighbor
  // 类型的元素嵌入到链表中，并在元素中添加了一个指向下一个元素的指针 pc_link。
  LIST_ENTRY(AODV_Neighbor)
  nb_link; // 将邻居缓存项链入链表中，以便于管理和维护。
  nsaddr_t nb_addr; // 邻居节点IP地址
  // 表示邻居节点信息的有效期，数据类型为 double。具体来说，该值等于
  // ALLOWED_HELLO_LOSS * HELLO_INTERVAL 其中 ALLOWED_HELLO_LOSS 表示允许的
  // HELLO 包损失次数，而 HELLO_INTERVAL 表示 HELLO 包的发送间隔时间。
  // 当邻居节点在连续多个 HELLO 包周期中都没有发送 HELLO
  // 包时，该邻居节点将被认为已经失效，其信息将从邻居缓存中删除。
  double nb_expire; // ALLOWED_HELLO_LOSS * HELLO_INTERVAL
};

/*
  这段代码定义了一个名为 aodv_ncache
  的链表头（LIST_HEAD），它是一个结构体类型，包含两个成员：
        lh_first：指向链表的第一个元素。
        lh_last：指向链表的最后一个元素。
  LIST_HEAD(aodv_ncache, AODV_Neighbor) 定义了一个以 AODV_Neighbor
  类型作为元素的链表头 aodv_ncache。 该链表头包含了一个指向链表第一个元素的指针
  lh_first 和一个指向链表最后一个元素的指针 lh_last。
*/
...
/*
  AODV Precursor list data structure
        ...
        当节点收到 RREQ
  消息中的源节点不在其前驱节点列表中，则将源节点添加到前驱节点列表中，并向其发送
  RREP 消息。
