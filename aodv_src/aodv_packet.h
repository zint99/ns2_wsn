...
// RREQ 路由请求数据包结构：<rq_src; rq_src_seqno; rq_bcast_id; rq_dst;
// rq_dst_seqno; rq_hop_count> 通过<rq_src,rq_bcast_id>唯一确定一个路由请求
// 当一个中间节点收到之前已经收到具有相同source_addr和broadcast_id的RREQ，则抛弃多余收到的RREQ而不会重新广播多余的RREQ
struct hdr_aodv_request {
  u_int8_t rq_type;     // Packet Type
  u_int8_t reserved[2]; // 保留字段，暂时未使用
  u_int8_t
      rq_hop_count; // Hop Count     跳数计数器：relay node转发RREQ时先将跳数+1
  ...
  double
      rq_timestamp; // when REQUEST sent;
                    // used to compute route discovery latency 计算路由发现时延

  // This define turns on gratuitous replies- see aodv.cc for implementation
  // contributed by Anant Utgikar, 09/16/02. #define RREQ_GRAT_RREP	0x80
  // 这个定义打开无偿回复gratuitous replies-见aodv.cc实现
...
/*
    这段代码定义了一个名为 hdr_all_aodv 的联合体，该联合体包含了 AODV
   协议中所有数据包类型的头部结构体，包括：ah、rreq、rrep、rerr、rrep_ack。
    联合体的作用是在对 AODV
   协议进行头部空间预留时，可以通过统一的结构体类型来计算预留空间的大小。
    计算预留空间大小
*/
union hdr_all_aodv {
  hdr_aodv ah;
